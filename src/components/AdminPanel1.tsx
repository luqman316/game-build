...
export default function AdminPanel1() {
  ...
  // Dummy credentials
  const ADMIN_EMAIL = "admin@gamedev1.com";
  ...
  const fetchCodes = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:4000/api/get-codes");
      const data = await response.json();

      if (data.success) {
        setCodes(data.data.codes || []);
        setShowCodes(true);
        console.log(`📊 Loaded ${data.data.codes?.length || 0} codes`);
      } else {
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 mb-6 shadow-2xl">
          <div className="flex justify-between items-center">
            <h1 className="text-3xl font-bold text-white">
              🎮 Game Codes Admin Panel for Testing
            </h1>
            <button
